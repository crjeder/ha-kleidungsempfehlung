...
# =============================================================================
# Physiological Helper Functions
# =============================================================================

def calculate_bsa(height_m: float, weight_kg: float) -> float:
    """
...
def adjust_met_for_demographics(
    base_met: float,
    age: int | None = None,
    is_female: bool = False
) -> float:
    """
    Adjust metabolic rate for age and gender.

    Women have approximately 10% lower basal metabolic rate due to
    body composition differences. Elderly (65+) have reduced metabolic
    efficiency.

    Args:
        base_met: Base metabolic rate [Met]
        age: Age in years (optional)
        is_female: True for female, False for male

    Returns:
        Adjusted metabolic rate [Met]
    """
    factor = 1.0

    if is_female:
        factor -= 0.10  # ~10% lower basal metabolic rate

    if age is not None and age >= 65:
        factor -= 0.10  # ~10% reduction for elderly
...
@dataclass
class Weather:
    """Environmental conditions for clothing recommendation."""
    t_ambient: float              # Air temperature [°C]
    wind_speed_ms: float = 0.0    # Wind speed [m/s]
    rel_humidity: float = 50.0    # Relative humidity [%]
    rain_mm_h: float = 0.0        # Rain intensity [mm/h]
    t_radiant: float | None = None  # Mean radiant temperature [°C], default = t_ambient

    @property
    def t_radiant_effective(self) -> float:
        """Return t_radiant or t_ambient if not set."""
        return self.t_radiant if self.t_radiant is not None else self.t_ambient
...
def calculate_pmv_ppd(weather: Weather, met_rate: float, clo: float) -> tuple[float, float]:
    ...
    Note:
        ...
        does not directly affect the PMV value. Individual differences should
        be accounted for via the metabolic rate (use adjust_met_for_demographics).
...
def solve_clo_for_pmv_target(weather: Weather, met_rate: float,
    ...
    pmv_at_min, _ = calculate_pmv_ppd(weather, met_rate, clo_min)
    pmv_at_max, _ = calculate_pmv_ppd(weather, met_rate, clo_max)
    ...
    for _ in range(50):  # Max iterations
        ...
        pmv_mid, _ = calculate_pmv_ppd(weather, met_rate, clo_mid)
...
class RecommendationResult:
    """Result of clothing recommendation."""
    ensemble: Ensemble            # Optimized clothing ensemble
    achieved_clo: float           # Actual CLO value achieved
    target_clo: float             # Target CLO for primary (cold) conditions
    target_clo_warm: float | None  # Target CLO for warm conditions (if temp range)
    pmv: float                    # Achieved PMV value
...
class SmartClothingEngine:
    ...
    Example:
        ...
        result = engine.recommend_outfit(
            weather=Weather(t_ambient=8, wind_speed_ms=4),
            met_rate=2.0
        )
    """
    ...
    def __init__(self, inventory: Inventory,
        ...
        Args:
            ...
            solver: Optimization solver to use: "ilp" (optimal) or "heuristic" (fast).
        ...
        """
        if solver not in ("ilp", "heuristic"):
            raise ValueError(f"Invalid solver '{solver}'. Must be 'ilp' or 'heuristic'.")
    ...
    def calculate_target_clo(self, weather: Weather, met_rate: float,
    ...
    def calculate_pmv_ppd(self, weather: Weather, met_rate: float,
                          clo: float) -> tuple[float, float]:
        """
        ...
        Returns:
            ...
            - PPD: Predicted Percentage Dissatisfied [%]
        """
        pmv, ppd = calculate_pmv_ppd(weather, met_rate, clo)
    ...
    def recommend_outfit(
        self,
        weather: Weather,
        weather_high: Weather | None = None,
        met_rate: float = 1.2,
        pmv_target: float = 0.0,
        base_ensemble: Ensemble | None = None,
    ) -> RecommendationResult:
        """
        Main entry point for clothing recommendation.

        Calculates optimal clothing for given weather conditions. If weather_high
        is provided, uses two-phase optimization: first optimizes for warm conditions,
        then locks that outfit and adds layers for cold conditions.

        Args:
            weather: Primary weather conditions (cold/main conditions)
            weather_high: Optional warm weather conditions for layering strategy
            met_rate: Metabolic rate [Met] (default: 1.2, light office work)
            pmv_target: Target PMV value (default: 0 = thermal neutrality)
            base_ensemble: Optional pre-configured ensemble with locked items

        Returns:
            RecommendationResult with optimized ensemble and metrics
        """
        # Calculate target CLO for primary (cold) conditions
        target_clo = self.calculate_target_clo(weather, met_rate, pmv_target)

        target_clo_warm = None
        has_temp_range = weather_high is not None

        if has_temp_range:
            # Calculate target CLO for warm conditions
            target_clo_warm = self.calculate_target_clo(weather_high, met_rate, pmv_target)

            # Phase 1: Optimize for warm temperature (minimal clothing)
            # Use max rain from both conditions for waterproof constraint
            rain_mm_h = max(weather.rain_mm_h, weather_high.rain_mm_h)

            if self.solver == "ilp":
                ensemble_warm = self.optimize_ensemble_ilp(
                    target_clo=target_clo_warm,
                    rain_mm_h=rain_mm_h,
                    base_ensemble=base_ensemble
                )
            else:
                ensemble_warm = self.optimize_ensemble(
                    target_clo=target_clo_warm,
                    temp_fluctuation=0,
                    rain_mm_h=rain_mm_h,
                    base_ensemble=base_ensemble
            ...
            # Phase 2: Lock warm outfit, add layers for cold
            locked_ensemble = {slot: [None] * self.max_layers for slot in SLOTS}
            for slot in SLOTS:
                for idx, entry in enumerate(ensemble_warm.get(slot, [])):
                    item_id = get_item_id(entry)
                    if item_id:
                        locked_ensemble[slot][idx] = make_entry(item_id, locked=True)

            if self.solver == "ilp":
                ensemble = self.optimize_ensemble_ilp(
                    target_clo=target_clo,
                    rain_mm_h=rain_mm_h,
                    base_ensemble=locked_ensemble
                )
            else:
                ensemble = self.optimize_ensemble(
                    target_clo=target_clo,
                    temp_fluctuation=0,
                    rain_mm_h=rain_mm_h,
                    base_ensemble=locked_ensemble
            ...
            # Single temperature optimization
            if self.solver == "ilp":
                ensemble = self.optimize_ensemble_ilp(
                    target_clo=target_clo,
        ...
        # Calculate achieved CLO and PMV
        achieved_clo = self._calc_current_clo(ensemble, self.inventory)
        pmv, ppd = self.calculate_pmv_ppd(weather, met_rate, achieved_clo)

        return RecommendationResult(
            ensemble=ensemble,
            achieved_clo=achieved_clo,
            target_clo=target_clo,
            target_clo_warm=target_clo_warm,
            pmv=pmv,
            ppd=ppd
        )
